...
public class ChangeWagonAction extends Action {
    ...
    public void apply(Bandit bandit) {
        if(!this.canApply(bandit)) {
            if (bandit.getToigon().getVoisin(dir)==null || !bandit.getToigon().getVoisin(dir).getEntites(Entite.Type.SHERIFF).isEmpty()){
                System.out.println(bandit.getNom() + " est bloqué par le Shérif");
            }
    ...
    public boolean canApply(Bandit bandit) {
        return (bandit.getToigon().getVoisin(this.dir)==null || bandit.getToigon().getVoisin(dir).getEntites(Entite.Type.SHERIFF).isEmpty());
    }
}
